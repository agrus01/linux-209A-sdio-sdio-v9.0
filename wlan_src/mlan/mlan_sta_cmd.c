		      ...
		      IN t_u16 cmd_action, IN t_void *pdata_buf)
{
	...
	/* ie_buf size is 262*4 = 1048 */
	t_u8 ie_buf[sizeof(custom_ie) * MAX_MGMT_IE_INDEX_TO_FW];
	...
	memset(pmpriv->adapter, ie_buf, 0,
	       (sizeof(custom_ie) * MAX_MGMT_IE_INDEX_TO_FW));
	memcpy(pmpriv->adapter, ie_buf, cust_ie->ie_data_list, req_len);
	travel_len = 0;
	...
	while (req_len > sizeof(t_u16)) {
		cptr = (custom_ie *)(ie_buf + travel_len);
	...
	memcpy(pmpriv->adapter, cust_ie->ie_data_list, ie_buf, cust_ie->len);
	if (cust_ie->len)
			 ...
			 IN t_u16 cmd_action, IN t_void *pdata_buf)
{
	...
	if (cmd_action == HostCmd_ACT_GEN_SET) {
		...
		for (cnt = 0; cnt < cfg->num_of_rules; cnt++) {
			...
			param = rule->params;
			for (idx = 0;
			     (idx < cfg->rule[cnt].num_of_fields) &&
			     (idx < COALESCE_MAX_FILTERS); idx++) {
				param->operation =
			 ...
			 */
			rule->header.len =
				(t_u16)wlan_cpu_to_le16(length + sizeof(t_u16) +
							sizeof(t_u8) +
							sizeof(t_u8));
