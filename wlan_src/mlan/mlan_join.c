			      ...
			      OUT t_u8 *pout_rates, OUT t_u32 *pout_rates_size)
{
	...
	memset(pmpriv->adapter, card_rates, 0, WLAN_SUPPORTED_RATES);
 ...
 */
t_u16
wlan_get_rsn_cap(t_u8 *data, t_u8 len)
{
	t_u16 rsn_cap = 0;
	t_u16 *ptr;
	t_u16 *end_ptr;
...
#define PREFERENCE_TKIP     1
	...
	t_u8 akm_preference[19] =
		{ 0, 0, 1, 0, 0, 0, 2, 0, 5, 0, 0, 0, 0, 0, 0, 0, 0, 0, 4 };
	mlan_adapter *pmadapter = pmpriv->adapter;
	...
	if ((akm_suite_count == 0) || (ptr + AKM_SUITE_LEN * akm_suite_count + sizeof(t_u16)) > end_ptr) {	//sizeof(t_u16) is for rsncap
		PRINTM(MERROR, "RSNE: AKM Suite or RSNCAP not correct\n");
		return MLAN_STATUS_FAILURE;
	}
			...
			if (ptr[3] < sizeof(akm_preference)) {
				if ((*akm_type == AssocAgentAuth_Open) &&
					...
					break;
					   ...
					   (ptr[3] == 2)) {
					break;
				} else if ((*akm_type == AssocAgentAuth_Wpa3Sae)
					   && (ptr[3] == 8)) {
					break;
				}
			}
	...
	*prsn_cap &= pmf_mask;
	...
	if (end_ptr >= (ptr + sizeof(t_u16))) {
		...
		if (pmkid_count && (end_ptr >= (ptr + pmkid_count * PMKID_LEN))) {
			pmkid_list_ptr = ptr;
