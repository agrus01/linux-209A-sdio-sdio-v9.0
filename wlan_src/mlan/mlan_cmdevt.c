...
wlan_process_cmdresp(mlan_adapter *pmadapter)
{
	...
	if ((pmadapter->hw_status == WlanHardwareStatusInitializing) &&
		...
		while ((i < pmadapter->priv_num) && (i < MLAN_MAX_BSS_NUM) &&
			       ...
			       IN t_u16 cmd_action, IN t_void *pdata_buf)
{
	...
	if (psk->psk_type == MLAN_PSK_SAE_PASSWORD) {
		...
		memcpy(pmpriv->adapter, psae_password_tlv->sae_password,
		       psk->psk.sae_password.sae_password,
		       MIN(psk->psk.sae_password.sae_password_len,
