				  ...
				  pmlan_private *priv, int *tid)
{
	...
	PRINTM(MDAT_D, "POP\n");
	for (j = pmadapter->priv_num - 1; (j >= 0) && (j < MLAN_MAX_BSS_NUM);
	     --j) {
		if (!
		    (util_peek_list
		     (pmadapter->pmoal_handle,
		      &pmadapter->bssprio_tbl[j].bssprio_head, MNULL, MNULL)))
			continue;
...
wlan_wmm_lists_empty(pmlan_adapter pmadapter)
{
	...
	for (j = 0; (j < pmadapter->priv_num) && (j < MLAN_MAX_BSS_NUM); ++j) {
		priv = pmadapter->priv[j];
		if (priv) {
				 ...
				 IEEEtypes_WmmParameter_t *pwmm_ie)
{
	...
	if ((priv->wmm_required && pwmm_ie->vend_hdr.element_id == WMM_IE)
		...
		if (priv->wmm_required
			...
			pwmm_tlv->header.type =
				wlan_cpu_to_le16(pwmm_tlv->header.type);
			...
			ret_len =
				sizeof(pwmm_tlv->header) + pwmm_tlv->header.len;
			pwmm_tlv->header.len =
				wlan_cpu_to_le16(pwmm_tlv->header.len);
