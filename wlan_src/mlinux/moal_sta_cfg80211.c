		       ...
		       mlan_ssid_bssid *ssid_bssid)
{
	...
	if (req->ie && req->ie_len) {
		...
		if (priv->sme_current.ie == NULL) {
			PRINTM(MERROR,
			       "Fail to allocate sme_current ie buffer\n");
			LEAVE();
			return;
		}
			...
			struct cfg80211_assoc_request *req)
{
	...
	if (req->prev_bssid) {
		memcpy(ssid_bssid.prev_bssid, req->prev_bssid, ETH_ALEN);
	}
		    ...
		    mlan_ds_misc_assoc_rsp *assoc_rsp)
{
	...
	if (conn_param && conn_param->prev_bssid) {
		memcpy(ssid_bssid.prev_bssid, conn_param->prev_bssid, ETH_ALEN);
	}
#endif
...
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)
	    && (priv->wdev->connected != NULL)) {
#else
	    && (priv->wdev->current_bss != NULL)) {
#endif
...
woal_cfg80211_resume(struct wiphy *wiphy)
{
	...
	memset(&wakeup_reason, 0, sizeof(mlan_ds_hs_wakeup_reason));
	if (!priv) {
...
woal_save_conn_params(moal_private *priv, struct cfg80211_connect_params *sme)
{
	...
	if (sme->ie && sme->ie_len) {
		priv->sme_current.ie = kzalloc(sme->ie_len, GFP_ATOMIC);
		if (priv->sme_current.ie == NULL) {
			PRINTM(MERROR,
			       "Fail to allocate sme_current ie buffer\n");
			LEAVE();
			      ...
			      u8 *mac, struct station_parameters *params)
{
	...
	if (params->supported_rates_len) {
		...
		memcpy(tlv->data, params->link_sta_params.supported_rates,
		       tlv->header.len);
#else
...
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(6, 0, 0)
		memcpy(tlv->data, params->link_sta_params.ht_capa,
		       tlv->header.len);
#else
...
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(6, 0, 0)
		memcpy(tlv->data, params->link_sta_params.vht_capa,
		       tlv->header.len);
#else
